...
__attr_nodiscard __attr_hot static inline int from_hex(const char *hex, unsigned char * __restrict__ out, size_t outlen) __noexcept {
    size_t len = strlen(hex);
    if (unlikely(len % 2 != 0 || outlen < len/2)) return 0;
...
__attr_hot static inline void RC2_key_expand(RC2_CTX * __restrict__ ctx, const unsigned char * __restrict__ key, size_t keylen) __noexcept {
...
__attr_hot static inline void RC2_block_encrypt(const RC2_CTX *ctx, unsigned char * __restrict__ block) __noexcept {
    uint16_t x[4];
    for (int i = 0; i < 4; ++i)
...
__attr_hot static inline void RC2_block_decrypt(const RC2_CTX *ctx, unsigned char * __restrict__ block) __noexcept {
    uint16_t x[4];
    for (int i = 0; i < 4; ++i)
        x[i] = block[2*i] + (block[2*i+1] << 8);
    int j = 63;
...
__attr_nodiscard __attr_hot static inline size_t RC2_Encrypt(const char *key, const char *plaintext, unsigned char * __restrict__ ciphertext) __noexcept {
    RC2_CTX ctx;
...
__attr_nodiscard __attr_hot static inline size_t RC2_Decrypt(const char *key, const unsigned char *ciphertext, size_t ctlen, unsigned char * __restrict__ plaintext) __noexcept {
    RC2_CTX ctx;
    size_t keylen = strlen(key);
    RC2_key_expand(&ctx, (const unsigned char*)key, keylen);
...
__attr_hot static inline void RC4_set_key(RC4_CTX * __restrict__ ctx, const unsigned char * __restrict__ key, size_t keylen) __noexcept {
    for (int i = 0; i < 256; ++i)
        ctx->S[i] = (uint8_t)i;
    uint8_t j = 0;
    for (int i = 0; i < 256; ++i) {
...
__attr_hot static inline void RC4_process(RC4_CTX * __restrict__ ctx, unsigned char * __restrict__ data, size_t length) __noexcept {
...
__attr_hot static inline void RC4_Encrypt(const char *key, const char *plaintext, unsigned char * __restrict__ ciphertext) __noexcept {
    RC4_CTX ctx;
    size_t keylen = strlen(key);
    size_t ptlen = strlen(plaintext);
...
__attr_hot static inline void RC4_Decrypt(const char *key, const unsigned char *ciphertext, size_t ctlen, unsigned char * __restrict__ plaintext) __noexcept {
    RC4_CTX ctx;
    size_t keylen = strlen(key);
    RC4_set_key(&ctx, (const unsigned char*)key, keylen);
...
__attr_hot static inline uint32_t RC5_rotl(uint32_t x, uint32_t y) __noexcept { return (x << (y & 31)) | (x >> (32 - (y & 31))); }
__attr_hot static inline uint32_t RC5_rotr(uint32_t x, uint32_t y) __noexcept { return (x >> (y & 31)) | (x << (32 - (y & 31))); }

__attr_hot static inline void RC5_key_expand(RC5_CTX * __restrict__ ctx, const unsigned char * __restrict__ key, size_t keylen, uint32_t rounds) __noexcept {
...
__attr_hot static inline void RC5_block_encrypt(const RC5_CTX *ctx, unsigned char * __restrict__ block) __noexcept {
    uint32_t A, B;
    memcpy(&A, block, 4);
    memcpy(&B, block+4, 4);
    A += ctx->S[0]; B += ctx->S[1];
...
__attr_hot static inline void RC5_block_decrypt(const RC5_CTX *ctx, unsigned char * __restrict__ block) __noexcept {
    uint32_t A, B;
    memcpy(&A, block, 4);
    memcpy(&B, block+4, 4);
...
__attr_nodiscard __attr_hot static inline size_t RC5_Encrypt(const char *key, const char *plaintext, unsigned char * __restrict__ ciphertext, uint32_t rounds) __noexcept {
    RC5_CTX ctx;
    size_t keylen = strlen(key);
    size_t ptlen = strlen(plaintext);
...
__attr_nodiscard __attr_hot static inline size_t RC5_Decrypt(const char *key, const unsigned char *ciphertext, size_t ctlen, unsigned char * __restrict__ plaintext, uint32_t rounds) __noexcept {
    RC5_CTX ctx;
    size_t keylen = strlen(key);
    RC5_key_expand(&ctx, (const unsigned char*)key, keylen, rounds);
...
__attr_hot static inline uint32_t RC6_rotl(uint32_t x, uint32_t y) __noexcept { return (x << (y & 31)) | (x >> (32 - (y & 31))); }
__attr_hot static inline uint32_t RC6_rotr(uint32_t x, uint32_t y) __noexcept { return (x >> (y & 31)) | (x << (32 - (y & 31))); }

__attr_hot static inline uint32_t RC6_get_u32(const unsigned char *b) __noexcept {
    return ((uint32_t)b[0]) | (((uint32_t)b[1]) << 8) | (((uint32_t)b[2]) << 16) | (((uint32_t)b[3]) << 24);
}
__attr_hot static inline void RC6_set_u32(unsigned char *b, uint32_t v) __noexcept {
    b[0] = v & 0xFF; b[1] = (v >> 8) & 0xFF; b[2] = (v >> 16) & 0xFF; b[3] = (v >> 24) & 0xFF;
}

__attr_hot static inline void RC6_key_expand(RC6_CTX * __restrict__ ctx, const unsigned char * __restrict__ key, size_t keylen, uint32_t rounds) __noexcept {
    const uint32_t Pw = 0xB7E15163, Qw = 0x9E3779B9;
    size_t Llen = (keylen+3)/4;
    uint32_t* L = (uint32_t*)calloc(Llen ? Llen : 1, sizeof(uint32_t));
    for (int i = keylen-1; i >= 0; --i)
...
__attr_hot static inline void RC6_block_encrypt(const RC6_CTX *ctx, unsigned char * __restrict__ block) __noexcept {
...
__attr_hot static inline void RC6_block_decrypt(const RC6_CTX *ctx, unsigned char * __restrict__ block) __noexcept {
    uint32_t A = RC6_get_u32(block);
    uint32_t B = RC6_get_u32(block+4);
    uint32_t C = RC6_get_u32(block+8);
    uint32_t D = RC6_get_u32(block+12);
...
__attr_nodiscard __attr_hot static inline size_t RC6_Encrypt(const char *key, const char *plaintext, unsigned char * __restrict__ ciphertext, uint32_t rounds) __noexcept {
    RC6_CTX ctx;
    size_t keylen = strlen(key);
    size_t ptlen = strlen(plaintext);
...
__attr_nodiscard __attr_hot static inline size_t RC6_Decrypt(const char *key, const unsigned char *ciphertext, size_t ctlen, unsigned char * __restrict__ plaintext, uint32_t rounds) __noexcept {
    RC6_CTX ctx;
    size_t keylen = strlen(key);
    RC6_key_expand(&ctx, (const unsigned char*)key, keylen, rounds);
